"""
Le-JEPA Anti-Collapse Trainer (Border Artifact Fixed)
- 표현 붕괴 방지 로직 유지
- PCA 시각화 시 발생하는 '테두리(Border) 노이즈' 현상을 Replication Padding으로 해결
"""
...
def save_spatial_pca_visualization(original_img, full_latents, patch_size=16, filename="pca_no_border.png"):
    ...
    # [수정된 부분] 0으로 채우는 제로 패딩 대신, 가장자리 픽셀을 거울처럼 복사(Replicate)하여 테두리 노이즈 원천 차단
    padded_features = F.pad(features_tensor, pad=(1, 1, 1, 1), mode='replicate')
    # 패딩을 이미 수동으로 했으므로 avg_pool2d에서는 padding=0 사용

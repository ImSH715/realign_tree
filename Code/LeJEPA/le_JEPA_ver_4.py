...
def apply_block_masking(img_tensor, patch_size=16, mask_ratio=0.0005):
...
def save_spatial_pca_visualization(original_img, full_latents, patch_size=16, filename="pca_no_border.png"):
    ...
    axes[1].axis('off')
    axes[1].set_title("Features (Border Artifact Fixed)", fontsize=14)
    
    plt.tight_layout()
    plt.savefig(filename, dpi=200, bbox_inches='tight')
    plt.close()
    print(f"\n[성공] 테두리가 제거된 PCA 시각화 저장됨: '{filename}'")

def main():
    TARGET_SIZE = 448
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using : {device} | Image size : {TARGET_SIZE}")
    ...
    try:
        ...
        if USE_CUSTOM_AREA:
            ...
            if right <= raw_pil_image.size[0] and lower <= raw_pil_image.size[1]:
                ...
                print(f"-> 지정된 영역 {crop_box} 으로 이미지가 잘렸습니다. (새 해상도: {raw_pil_image.size})")
            else:
    ...
    with torch.no_grad():
        ...
        # (테스트용 새로운 마스크 생성 로직 추가 - 모양 충돌 방지)
        ...
        save_spatial_pca_visualization(test_img.cpu(), full_latents.cpu(), patch_size=16, filename="leJEPA_data/improved_pca_rgb.png")

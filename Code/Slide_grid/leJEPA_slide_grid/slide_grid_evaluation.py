...
from pyproj import Transformer  # <-- [핵심] 322개의 좌표 꼬임을 막아줄 구원투수
    ...
    for tif_path in tqdm(tif_files, desc="Processing TIFs"):
        try:
            with rasterio.open(tif_path) as src:
                ...
                # [안전장치] TIF 좌표계에서 계산된 좌표를 다시 원본 좌표계(32718)로 되돌려줍니다.
                transformer_to_target = Transformer.from_crs(src.crs, gdf.crs, always_xy=True)
                ...
                for idx, row in inner_pbar:
                    ...
                    current_x, current_y = row.geometry.x, row.geometry.y
                    orig_target_geom = gdf.loc[idx].geometry
                    orig_target_x, orig_target_y = orig_target_geom.x, orig_target_geom.y
                    ...
                    if label_val not in ref_embeddings:
                        ...
                        for step in range(1, MAX_STEPS + 1):
                            ...
                            point_trajectories[fid][step] = {"x": orig_target_x, "y": orig_target_y, "status": status_val}
                        continue
                    ...
                    for step in range(1, MAX_STEPS + 1):
                        ...
                        if num_checked >= 3:
                            ...
                            cx_tgt, cy_tgt = transformer_to_target.transform(cx, cy) 
                            
                            point_trajectories[fid][step] = {"x": cx_tgt, "y": cy_tgt, "status": "center"}
                            ...
                            if step == MAX_STEPS:
                                ...
                                point_trajectories[fid][step] = {"x": cx_tgt, "y": cy_tgt, "status": "slide"}
                                
                        else:
                            curr_tgt_x, curr_tgt_y = transformer_to_target.transform(current_x, current_y)
                            if step == MAX_STEPS:
                                point_trajectories[fid][step] = {"x": curr_tgt_x, "y": curr_tgt_y, "status": "center"}
                                real_status_tracker[fid] = "zero_near_tree_fail"
                            else:
                                point_trajectories[fid][step] = {"x": curr_tgt_x, "y": curr_tgt_y, "status": "zero_near_tree_fail"}
                                real_status_tracker[fid] = "zero_near_tree_fail"

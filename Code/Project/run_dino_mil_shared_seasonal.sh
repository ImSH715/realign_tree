...
#SBATCH --output=logs/dino_mil_20m_%j.out
#SBATCH --error=logs/dino_mil_20m_%j.err
...
OUT_NAME="${OUT_NAME:-mil_dino_shared_seasonal_20m_${TRAIN_IMAGE_MODE}${RUN_TAG}_cuda}"
...
elif [ -f "./outputs/binary_dino_shared_seasonal_rgb_balanced_cuda_80ep/phase1_encoder_best.pth" ]; then
  INIT_ENCODER_CKPT="./outputs/binary_dino_shared_seasonal_rgb_balanced_cuda_80ep/phase1_encoder_best.pth"
elif [ -f "./outputs/binary_dino_shared_seasonal_rgb_balanced_cuda/phase1_encoder_best.pth" ]; then
  INIT_ENCODER_CKPT="./outputs/binary_dino_shared_seasonal_rgb_balanced_cuda/phase1_encoder_best.pth"
elif [ -f "./outputs/phase1_dino_ssl_shared_seasonal/phase1_encoder_best.pth" ]; then
  INIT_ENCODER_CKPT="./outputs/phase1_dino_ssl_shared_seasonal/phase1_encoder_best.pth"
else
  echo "Could not find an init encoder checkpoint."
  echo "Set INIT_CKPT explicitly, or run seasonal DINO / seasonal binary first."
  exit 1
fi

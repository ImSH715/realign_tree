...
LOG_DIR="${LOG_DIR:-logs/mil_factorial}"
...
for job_id in "$@"; do
  ...
  while IFS= read -r path; do
    matches+=("$path")
  done < <(find "$LOG_DIR" -maxdepth 1 -type f \( -name "*_${job_id}.out" -o -name "*_${job_id}.err" \) 2>/dev/null | sort)

  if [ "${#matches[@]}" -eq 0 ]; then
    echo "No logs found under $LOG_DIR for job $job_id."

...
class Phase3Config:
    ...
    x_column: str = "x"
    y_column: str = "y"
    target_label_column: str = "corrected_label"
...
def load_points_from_generic_csv(
    csv_path: str,
    x_column: str,
    y_column: str,
    label_column: str,
    ...
    if x_column not in df.columns:
        raise ValueError(f"x_column '{x_column}' not found in CSV. Available columns: {df.columns.tolist()}")
    if y_column not in df.columns:
        raise ValueError(f"y_column '{y_column}' not found in CSV. Available columns: {df.columns.tolist()}")

    if label_column:
        if label_column not in df.columns:
            raise ValueError(f"label_column '{label_column}' not found in CSV. Available columns: {df.columns.tolist()}")
        df[target_label_column] = df[label_column].astype(str)

    if filter_label:
        df = df[df[target_label_column].astype(str).str.lower() == filter_label.lower()].copy()

    if point_id_column not in df.columns:
        df[point_id_column] = [f"point_{i:06d}" for i in range(len(df))]

    if tile_column not in df.columns:
        if not default_image_path:
            raise ValueError(
                f"tile_column '{tile_column}' not found in CSV, and no --default_image_path was provided."
            )
        df[tile_column] = default_image_path
        ...
        points.append(
            {
                ...
                "x_raw": float(row[x_column]),
                "y_raw": float(row[y_column]),
                "target_label": str(row[target_label_column]),
...
def convert_world_to_pixel(points):
    converted = []
    for p in points:
        if p["coord_type"] == "pixel":
            converted.append(
                InputPoint(
                    point_id=p["point_id"],
        ...
        with rasterio.open(p["image_path"]) as src:
            row_px, col_px = src.index(p["x_raw"], p["y_raw"])
...
def parse_args():
    ...
    parser.add_argument("--filter_label", type=str, default="")
    parser.add_argument("--default_image_path", type=str, default="")
...
def main():
    ...
    raw_points, raw_df = load_points_from_generic_csv(
    ...
    points = convert_world_to_pixel(raw_points)

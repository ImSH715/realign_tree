...
For each labelled point, a bag of candidate crops is constructed. Positive bags sample candidate centres within a 20 m radius of the recorded point. During training, candidate offsets are sampled randomly within this radius, which exposes the model to multiple plausible crown positions. During validation and analysis, a deterministic set of offsets is used, beginning with the recorded centre and then sampling directions on several radial rings. In the current configuration, each bag contains 17 instances.

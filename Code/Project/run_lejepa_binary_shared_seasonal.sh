...
#SBATCH --job-name=lejepa_bin_season
...
#SBATCH --output=logs/mil/lejepa/phase_1_5/lejepa_bin_season_%j.out
#SBATCH --error=logs/mil/lejepa/phase_1_5/lejepa_bin_season_%j.err
...
TRAIN_OUT_NAME="binary_lejepa_shared_seasonal_rgb_balanced_cuda${RUN_TAG}"
EVAL_OUT_NAME="phase2_classifier_binary_lejepa_shared_seasonal_rgb_balanced_cuda${RUN_TAG}"
...
if [ -n "$SUPERVISED_INIT_DIR" ]; then
  ...
  PHASE1_DIR="./outputs/phase1_lejepa_ssl_large_gpu"
  if [ -f "$PHASE1_DIR/phase1_encoder_best.pth" ]; then
    ...
    echo "Missing LEJEPA SSL checkpoint under: $PHASE1_DIR"
    echo "Run run_lejepa_ssl_large.sh first."
    exit 1
  fi
fi
...
echo "Using LEJEPA init checkpoint: $INIT_CKPT"
if [ -n "$SUPERVISED_INIT_DIR" ]; then
...
echo "============================================================"
echo "Step 1: Fine-tuning binary classifier from seasonal LEJEPA encoder"
echo "============================================================"
...
echo "============================================================"
echo "Step 2: Evaluating seasonal LEJEPA binary classifier"
echo "============================================================"
